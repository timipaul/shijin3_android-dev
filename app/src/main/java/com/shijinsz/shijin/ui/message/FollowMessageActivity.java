...
import com.shijinsz.shijin.ui.message.adapter.FollowAdapter;
...
public class FollowMessageActivity extends BaseActivity implements OnRefreshListener, OnLoadMoreListener {
    ...
    private List<FollowBean.Messages> list = new ArrayList<>();
    ...
    public void initView(View view) {
        showTitleBackButton();
        setTitle(getString(R.string.user_like));
        ...
        int num = Integer.parseInt(ShareDataManager.getInstance().getPara(SharedPreferencesKey.KEY_user_like_number));
        ...
        ShareDataManager.getInstance().save(mContext, SharedPreferencesKey.KEY_user_like_number, "0");
        showTitleLeftText(getString(R.string.select_all), new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (list.size() > 0) {
                    boolean isALL = true;
                    for (FollowBean.Messages bean : list) {
                        if (!bean.isCheck()) {
                            isALL = false;
                        }
                    }
        ...
        adapter = new FollowAdapter(R.layout.follow_item, list);
...
//        map.put("message_ids", delete);
    ...
    public void getPoint(String cursor1) {
        ...
        map.put("mode", "person");
        map.put("size", "10");
        YSBSdk.getService(OAuthService.class).user_like(map, new YRequestCallback<FollowBean>() {
            @Override
            public void onSuccess(FollowBean var1) {
                mStateView.showContent();
