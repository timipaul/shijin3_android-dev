...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    ...
    private void drawOuterDecorateSmallCircle(Canvas canvas) {
        ...
        for(int i = 0; i < 9; i++) {
            ...
            y =  mOuterCircleWidth*2 + (sideSize - mSmallCircleRadius * 2 * 9) / 9 * i + mSmallCircleRadius * 2 * i + getPaddingTop()-18;
            canvas.drawCircle(x, y, mSmallCircleRadius, mSmallCirclePaint);
        }
    ...
    private void drawInnerBackground(Canvas canvas) {
        mInnerPaint.setColor(Color.parseColor("#815D01"));
        canvas.drawRect(mOuterCircleWidth + getPaddingLeft(),
                mOuterCircleWidth + getPaddingTop(),
                mSelfTotalWidth - mOuterCircleWidth - getPaddingRight(),
...
//            int space = mInnerCardWidth / 9;
        ...
        if(mStartAnimation && need) {
            ...
            canvas.drawRoundRect(new RectF(left, top, right, bottom), 12, 12, mInnerPaint);
            mInnerPaint.setColor(mInnerCardDefaultColor);
            canvas.drawRoundRect(new RectF(left+6, top+6, right-6, bottom-6), 12, 12, mInnerPaint);
        }
        // 绘制卡片中的图片
        int picHeight = 0;
...
//        }
        if (index==4){
//            if (type==1){
//                mInnerPaint.setColor(index == 4 ? Color.parseColor("#F01414") : mInnerCardTextColor);
//                mInnerPaint.setTextSize(mBigInfoTextSize);
//                int textX = left + (mInnerCardWidth/2 - getTextWidth("点击", mInnerPaint)/2);
//                int textY_1 = top + (mInnerCardWidth / 2 ) - 3;
//                int textY_2 = top + (mInnerCardWidth / 2 + getTextHeight("点击", mInnerPaint)) + 3;
//                canvas.drawText("点击", textX, textY_1, mInnerPaint);
//                canvas.drawText("抽奖", textX, textY_2, mInnerPaint);
//            }else {
                mInnerPaint.setColor(index == 4 ? Color.parseColor("#F01414") : mInnerCardTextColor);
                mInnerPaint.setTextSize(mBigInfoTextSize);
                int textX = left + (mInnerCardWidth/2 - getTextWidth("50金", mInnerPaint)/2) ;
                int textY_1 = top + (mInnerCardWidth / 2 ) - 3;
                int textY_2 = top + (mInnerCardWidth / 2 + getTextHeight("50金", mInnerPaint)) + 3;
                canvas.drawText("50金", textX, textY_1, mInnerPaint);
                canvas.drawText("币/次", textX, textY_2, mInnerPaint);
//            }
        }else if (index>4){
            if (list.size()>index-1 && list.get(index-1)!=null) {
                        Bitmap bitmap=list.get(index-1);
                        canvas.drawBitmap(bitmap, null, new RectF(left+6, top+6, right-6, bottom-6), mInnerPaint);
            }
...
//            mStartAnimation = false;
    ...
    private class ResultTask implements Runnable{
        ...
        public void run() {
            ...
            if (onClickListen!=null&&mLotteryInvalidateTimes!=0){
                onClickListen.onEnd(mLotteryInvalidateTimes-36);
            }
            mLotteryInvalidateTimes = 0;
...
//        builder.create().show();
    ...
    public void start(int mLotteryInvalidateTimes,int type){
        this.type=type;
        this.mLotteryInvalidateTimes=mLotteryInvalidateTimes;
        loopSmallCircleAnimation();
