...
//        int triangleX = posCenterPosition[0] + contentPosition[0];
        int triangleX = posCenterPosition[0];
        int halfTriangleWidth = mTranWidth / 2;
        if (isShowDown) {
            canvas.drawRoundRect(new RectF(mDrawMargin, 0 + mTranHeight, canvas.getWidth() - mDrawMargin, canvas.getHeight() - mDrawMargin - ScreenUtils.dp2px(getContext(), 10)), mRadius, mRadius, mPaint);

            mPath.moveTo(triangleX, 0);
            mPath.lineTo(triangleX + halfTriangleWidth, 0 + mTranHeight * 2);
            mPath.lineTo(triangleX - halfTriangleWidth, 0 + mTranHeight * 2);
            mPath.close();
            canvas.drawPath(mPath, mPaint);
        } else {
            canvas.drawRoundRect(new RectF(mDrawMargin, mDrawMargin, canvas.getWidth() - mDrawMargin, canvas.getHeight() - 0 - mTranHeight), mRadius, mRadius, mPaint);

            mPath.moveTo(triangleX, canvas.getHeight() - 0);
            mPath.lineTo(triangleX + halfTriangleWidth, canvas.getHeight() - 0 - mTranHeight * 2);
            mPath.lineTo(triangleX - halfTriangleWidth, canvas.getHeight() - 0 - mTranHeight * 2);
            mPath.close();
            canvas.drawPath(mPath, mPaint);
